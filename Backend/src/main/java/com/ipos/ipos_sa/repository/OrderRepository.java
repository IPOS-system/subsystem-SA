...
public interface OrderRepository extends JpaRepository<Order, String> {
  ...
  /** Lists all orders for a specific merchant, newest first. */
  List<Order> findByMerchant_MerchantIdOrderByOrderDateDesc(Integer merchantId);

  /** Filter by status. */
  List<Order> findByStatus(Order.OrderStatus status);

  /** Lists merchant order history within a date range. */
  ...
  /** All orders within a date range regardless of merchant. */
