...
public class OrderService {
   ...
   *
   * <p>Business rules enforced (per the brief): 1. Merchant account must be NORMAL — SUSPENDED and
  ...
  public OrderDTO placeOrder(Integer merchantId, PlaceOrderRequest request, User actingUser) {
    ...
    for (PlaceOrderRequest.OrderLineRequest line : request.getItems()) {
      ...
      // Normalize product ID: the DB stores "100 00001" (spaces) but the SA Endpoint
      // Reference docs show "100-00001" (hyphens). Accept either format.
   ...
   *
   * <p>For DISPATCHED, use the dispatchOrder() method instead — it requires courier details.
   */
  @Transactional
  ...
  /**
   * Marks an order as DISPATCHED and records the courier details. Per the brief, dispatch details
   * are required when transitioning to DISPATCHED.
   */
  @Transactional
  ...
  /**
   * Returns a single order by ID. Used by merchants to track their order and by admins to view
   * order detail.
   */
  ...
  /**
   * Returns all orders for a merchant, newest first. Used by the My Orders screen on the merchant
   * dashboard.
   */
  ...
  /** Returns all orders filtered by status. Used by the admin Past Orders screen. */
  ...
  /**
   * Returns all orders across all merchants. Used by the admin Past Orders screen when no filter is
   * applied.
   */
  ...
  /**
   * Returns all incomplete orders (not DELIVERED, not CANCELLED). Used by the admin "orders not
   * completed" view.
   */
  ...
  /**
   * Generates a unique order ID in the format ORD-YYYYMMDD-XXXX. The suffix is derived from the
   * current count of orders today + 1. For a prototype this is sufficient; production would use a
   * sequence.
   */
