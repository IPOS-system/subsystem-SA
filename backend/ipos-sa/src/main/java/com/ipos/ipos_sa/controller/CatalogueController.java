...
/**
 * REST controller for catalogue management (IPOS-SA-CAT).
 *
 * Role rules:
 *   GET  /api/catalogue             → any authenticated user (merchants browse here)
 *   GET  /api/catalogue/admin       → ADMIN, MANAGER only (includes inactive + stock internals)
 *   POST /api/catalogue             → ADMIN, MANAGER (add product)
 *   PUT  /api/catalogue/{id}        → ADMIN, MANAGER (update product)
 *   PUT  /api/catalogue/{id}/deactivate   → ADMIN, MANAGER (soft-delete)
 *   PUT  /api/catalogue/{id}/reactivate   → ADMIN, MANAGER (restore)
 *   POST /api/catalogue/{id}/stock  → ADMIN, MANAGER (record stock delivery)
 */
@RestController
...
public class CatalogueController {
    ...
    /**
     * GET /api/catalogue
     * Returns active products only. Optionally filter with ?search=term.
    ...
    public ResponseEntity<List<CatalogueItemDTO>> browseCatalogue(
        ...
        List<CatalogueItemDTO> result = (search != null && !search.isBlank())
    ...
    /**
     * GET /api/catalogue/admin
     * Returns all products with full admin detail. ADMIN/MANAGER only.
     */
    @GetMapping("/admin")
    ...
    /**
     * GET /api/catalogue/admin/{id}
     * Returns a single product with full admin detail.
     */
    @GetMapping("/admin/{id}")
    ...
    /**
     * POST /api/catalogue
     * Adds a new product to the catalogue (UC-10).
     */
    @PostMapping
     ...
     * Updates product details (UC-11). Partial update — only non-null fields applied.
     */
    @PutMapping("/{id}")
    ...
    /**
     * PUT /api/catalogue/{id}/deactivate
     * Soft-deletes a product (UC-12). Product disappears from merchant view
     * but remains in the database for order history.
     */
    ...
    /**
     * PUT /api/catalogue/{id}/reactivate
    ...
    /**
     * POST /api/catalogue/{id}/stock
     * Records a stock delivery — increases availability (UC-34).
     */
