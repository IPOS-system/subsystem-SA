...
public interface InvoiceRepository extends JpaRepository<Invoice, String> {
  ...
  /** Lists invoices that still have an outstanding balance. */
  List<Invoice> findByMerchant_MerchantIdAndPaymentStatusNot(
      Integer merchantId, Invoice.PaymentStatus status);

  /**
   * Account status check: find the oldest unpaid invoice for a merchant so we can calculate how
   * many days overdue the merchant is.
   */
  @Query(
      "SELECT i FROM Invoice i WHERE i.merchant.merchantId = :merchantId "
          + "AND i.paymentStatus <> 'PAID' "
          + "ORDER BY i.dueDate ASC")
  List<Invoice> findUnpaidByMerchantOrderByDueDateAsc(@Param("merchantId") Integer merchantId);

  /**
   * Payment reminder check: return true if the merchant has any invoice that is overdue by 1–15
   * days (still NORMAL state, but reminder needed).
   */
  @Query(
      "SELECT COUNT(i) > 0 FROM Invoice i WHERE i.merchant.merchantId = :merchantId "
          + "AND i.paymentStatus <> 'PAID' "
          + "AND i.dueDate < :today "
          + "AND i.dueDate >= :fifteenDaysAgo")
  boolean hasInvoicesDueForReminder(
      @Param("merchantId") Integer merchantId,
      @Param("today") LocalDate today,
      @Param("fifteenDaysAgo") LocalDate fifteenDaysAgo);

  /** Invoice list for a merchant within a date range. */
