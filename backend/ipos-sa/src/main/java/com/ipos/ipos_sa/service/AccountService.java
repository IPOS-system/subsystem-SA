...
public class AccountService {
  ...
  /**
   * Creates a new merchant account (User + Merchant records). Per the brief: if any required field
   * is missing the account must NOT be created. All required fields are validated by @Valid on the
   * request DTO before this method is called, so we only need to check business-level rules here.
   *
  ...
  public MerchantDTO createMerchantAccount(CreateMerchantRequest request, User actingUser) {
    ...
    DiscountPlan discountPlan =
        discountPlanRepository
            ...
            .orElseThrow(
                () -> new ResourceNotFoundException("DiscountPlan", request.getDiscountPlanId()));
  ...
  /**
   * Creates a new staff account (ADMIN, MANAGER, ACCOUNTANT, DIRECTOR). Only ADMIN users should
   * call this endpoint — enforced in the controller.
   *
   * @param request the incoming creation request
  ...
  /**
   * Soft-deletes an account by setting isActive = false. The record is retained in the database for
  ...
  /**
   * Updates the contact fields of a merchant account. Only non-null fields in the request are
   * applied (partial update).
   */
  @Transactional
  public MerchantDTO updateMerchantDetails(
      Integer merchantId, UpdateMerchantRequest request, User actingUser) {
  ...
  /**
   * Changes the role of an existing user account. Per the marking sheet: "change roles
   * (promote/demote) to level of access." The user's username and password are retained.
   */
  @Transactional
  ...
  /**
   * Resets a user's password. ADMIN only — enforced in the controller. Per the brief (UC-3), the
   * old password is invalidated and replaced.
   */
  @Transactional
  ...
  /**
   * Restores a merchant account from IN_DEFAULT to NORMAL. Per the brief: only DIRECTOR or ADMIN
   * can authorise this. Role check is enforced in the controller — this method trusts the caller.
   */
  @Transactional
  public MerchantDTO restoreFromDefault(Integer merchantId, User actingUser) {
    ...
    if (merchant.getAccountStatus() != Merchant.AccountStatus.IN_DEFAULT) {
      throw new ValidationException("Merchant account is not IN_DEFAULT — no restoration needed.");
    }
    ...
    log.info(
        "Merchant restored from IN_DEFAULT: merchantId={} by userId={}",
        merchantId,
  ...
  /**
   * Checks the merchant's oldest unpaid invoice and automatically transitions account status based
   * on how many days overdue it is:
   *
   * <p>0–15 days overdue → NORMAL (reminder flag set on login response, handled in AuthService)
   * 15–30 days overdue → SUSPENDED 30+ days overdue → IN_DEFAULT
   *
   * <p>Called on every merchant login and before every order placement. Does nothing if the account
   * is already IN_DEFAULT (requires manual restoration).
   */
  @Transactional
  public void checkAndUpdateAccountStatus(Integer merchantId) {
    ...
    List<Invoice> unpaid = invoiceRepository.findUnpaidByMerchantOrderByDueDateAsc(merchantId);
    ...
    long daysOverdue = java.time.temporal.ChronoUnit.DAYS.between(oldestDueDate, LocalDate.now());
    ...
    if (newStatus != merchant.getAccountStatus()) {
      log.info(
          "Merchant {} status changing from {} to {} ({} days overdue)",
          merchantId,
  ...
  public List<MerchantDTO> searchMerchants(String term) {
    return merchantRepository.searchByNameOrEmail(term).stream()
        .map(this::toMerchantDTO)
        .collect(Collectors.toList());
  }
  ...
  public List<StaffDTO> getAllStaff() {
    return userRepository.findByRoleNot(User.Role.MERCHANT).stream()
        .map(this::toStaffDTO)
        .collect(Collectors.toList());
  }
  ...
  public MerchantDTO toMerchantDTO(Merchant merchant) {
    return MerchantDTO.builder()
        ...
        .discountPlanId(
            merchant.getDiscountPlan() != null ? merchant.getDiscountPlan().getPlanId() : null)
        .discountPlanName(
            merchant.getDiscountPlan() != null ? merchant.getDiscountPlan().getPlanName() : null)
