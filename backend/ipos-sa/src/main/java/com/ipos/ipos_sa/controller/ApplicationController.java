...
import com.ipos.ipos_sa.service.PUEmailService;
...
/**
 * REST controller for commercial membership applications.
 *
 * Inter-subsystem: IPOS-PU calls POST /api/applications/commercial to submit a commercial
 * membership application.
 *
 * Admin: GET /api/applications to review pending applications, PUT
...
public class ApplicationController {
  ...
  private final PUEmailService puEmailService;

  //  submitCA 

  /**
   * POST /api/applications/commercial Receives a commercial membership application from IPOS-PU.
   *
   * Maps to: ICMAAPI.submitCA(CommercialApplicationForm)
   *
   * This endpoint is public and does not require authentication.
   *
   * Returns the application ID as confirmation (matching the interface return type of String).
   */
  @PostMapping("/commercial")
  public ResponseEntity<CommercialApplicationDTO> submitCommercialApplication(
      @Valid @RequestBody SubmitApplicationRequest request) {

    CommercialApplication application =
        CommercialApplication.builder()
    ...
    log.info(
        "Commercial application submitted: {} (ID={})",
  ...
  /** GET /api/applications Returns all applications, optionally filtered by ?status=PENDING. */
  @GetMapping
  public ResponseEntity<List<CommercialApplicationDTO>> listApplications(
    ...
    List<CommercialApplication> apps =
        (status != null)
            ? applicationRepository.findByStatusOrderBySubmittedAtDesc(status)
            : applicationRepository.findAllByOrderBySubmittedAtDesc();

    List<CommercialApplicationDTO> result =
        apps.stream().map(this::toDTO).collect(Collectors.toList());
  ...
  /**
   * PUT /api/applications/{id}/approve Approves a pending commercial application. The admin would
   * then create a merchant account separately via POST /api/accounts/merchant.
   */
  ...
  public ResponseEntity<CommercialApplicationDTO> approveApplication(
    ...
    CommercialApplication app =
        applicationRepository
            .findById(id)
            .orElseThrow(() -> new ResourceNotFoundException("CommercialApplication", id));
    ...
    // Send approval email via PU's SMTP service
    puEmailService.sendEmail(
        app.getEmail(),
        "InfoPharma Ltd — Commercial Membership Application Approved",
        "Dear " + app.getCompanyName() + ",\n\n"
            + "Your application for a commercial membership with InfoPharma Ltd has been approved.\n\n"
            + "Your IPOS-SA login credentials will be sent to you separately via registered post.\n\n"
  ...
  /** PUT /api/applications/{id}/reject Rejects a pending commercial application. */
  ...
  public ResponseEntity<CommercialApplicationDTO> rejectApplication(
    ...
    // Send rejection email via PU's SMTP service
    String reasonText = (reason != null && !reason.isBlank())
        ? "\n\nReason: " + reason
        : "";
    puEmailService.sendEmail(
        app.getEmail(),
        "InfoPharma Ltd — Commercial Membership Application Update",
        "Dear " + app.getCompanyName() + ",\n\n"
            + "We regret to inform you that your application for a commercial membership "
            + "with InfoPharma Ltd has not been approved at this time." + reasonText + "\n\n"
