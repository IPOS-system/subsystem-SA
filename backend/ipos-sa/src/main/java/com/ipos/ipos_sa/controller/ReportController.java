 ...
 *
 * All reports are MANAGER-only unless otherwise noted.
 * ADMIN also has access for operational convenience.
 *
 * Date parameters use ISO format: 2026-01-01T00:00:00
 *
 * Endpoints:
 *   GET /api/reports/turnover?from=&to=                             → RPT-01
 *   GET /api/reports/merchant-summary?merchantId=&from=&to=         → RPT-02
 *   GET /api/reports/merchant-detailed?merchantId=&from=&to=        → RPT-03
 *   GET /api/reports/invoice-list?merchantId=&from=&to=             → RPT-04
 *   GET /api/reports/all-invoices?from=&to=                         → RPT-05
 *   GET /api/reports/low-stock                                      → RPT-07
 */
@RestController
...
public class ReportController {
     ...
     * Quantity sold and revenue received per product within a date range.
     */
    @GetMapping("/turnover")
    ...
    /**
     * GET /api/reports/merchant-summary?merchantId=...&from=...&to=...
     * Order list for a specific merchant with totals row (Appendix 4).
     */
    ...
    public ResponseEntity<MerchantOrdersSummaryDTO> merchantOrdersSummary(
        ...
        return ResponseEntity.ok(
                reportService.generateMerchantOrdersSummary(merchantId, from, to));
    }

    // ── RPT-03: Merchant Detailed Report ──────────────────────────────────────

    /**
     * GET /api/reports/merchant-detailed?merchantId=...&from=...&to=...
     * Contact details as header, all orders with line item breakdown (Appendix 5).
     */
    ...
    public ResponseEntity<MerchantDetailedReportDTO> merchantDetailedReport(
        ...
        requireRole(auth, User.Role.MANAGER, User.Role.ADMIN);
        return ResponseEntity.ok(
                reportService.generateMerchantDetailedReport(merchantId, from, to));
    }

    // ── RPT-04: Invoice List (Single Merchant) ────────────────────────────────

    /**
     * GET /api/reports/invoice-list?merchantId=...&from=...&to=...
     * All invoices for a specific merchant within a date range.
     */
    ...
    public ResponseEntity<InvoiceListReportDTO> merchantInvoiceList(
        ...
        return ResponseEntity.ok(
                reportService.generateMerchantInvoiceList(merchantId, from, to));
    }

    // ── RPT-05: All Invoices ──────────────────────────────────────────────────

    /**
     * GET /api/reports/all-invoices?from=...&to=...
     * All invoices across all merchants within a date range.
     */
     ...
     * All active products below minimum stock level with recommended
     * order quantities (Appendix 3 / UC-46).
     */
