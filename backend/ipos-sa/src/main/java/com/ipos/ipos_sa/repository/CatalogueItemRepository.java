...
public interface CatalogueItemRepository extends JpaRepository<CatalogueItem, String> {
  ...
  /**
   * Keyword search on active products — matches product ID prefix or description substring. Used by
   * the catalogue search bar on the merchant browse screen.
   */
  @Query(
   ...
   * level. Used for the login warning and RPT-07. This cannot be expressed with Spring Data method
   * naming because it compares two columns, so a JPQL query is required.
   */
  @Query(
