...
public class DiscountPlanService {
  ...
  /**
   * Deletes a discount plan by ID. Per the marking sheet: "delete discount plan" (1 mark). The plan
   * must not be in use by any merchant — if it is, a ValidationException is thrown. Tiers are
   * cascade-deleted by the database (ON DELETE CASCADE on discount_tiers → discount_plans).
   */
  @Transactional
  public void deletePlan(Integer planId, User actingUser) {
    DiscountPlan plan =
        discountPlanRepository
            .findById(planId)
            .orElseThrow(() -> new ResourceNotFoundException("DiscountPlan", planId));

    // Check if any merchant is currently using this plan
    long usageCount =
        discountPlanRepository.countMerchantsUsingPlan(planId);
    if (usageCount > 0) {
      throw new com.ipos.ipos_sa.exception.ValidationException(
          "Cannot delete discount plan '"
              + plan.getPlanName()
              + "' — it is currently assigned to "
              + usageCount
              + " merchant(s). Reassign them first.");
    }
    ...
    audit(
        ...
        "Deleted discount plan: " + plan.getPlanName());

    log.info("Discount plan deleted: {} (planId={})", plan.getPlanName(), planId);
  }
