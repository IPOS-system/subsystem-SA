 ...
 *
 * <p>Role rules: GET /api/invoices/{id} → any authenticated user (own invoice for merchants) GET
...
public class InvoiceController {
   ...
   * summaries. Powers the "My Balance" screen (merchant) and the Accountant's invoice list filtered
   * by merchant.
   *
   * <p>Merchants can only query their own invoices.
   */
   ...
   * balance for a merchant. Used by the Accountant's "Record Payment" form to populate the invoice
   * dropdown — only invoices that still need payment are shown.
   */
