 ...
 *
 * <p>Role rules: GET /api/discount-plans → ADMIN, MANAGER (populate dropdowns) GET
 * /api/discount-plans/{id} → ADMIN, MANAGER (view plan detail) POST /api/discount-plans/fixed →
 * ADMIN, MANAGER (create fixed plan) POST /api/discount-plans/flexible → ADMIN, MANAGER (create
 * flexible plan)
 */
@RestController
...
public class DiscountPlanController {
  ...
  /**
   * GET /api/discount-plans Returns all discount plans. Used to populate the dropdown on merchant
   * account creation and edit screens.
   */
  @GetMapping
  ...
  /** GET /api/discount-plans/{id} Returns a single plan with full tier detail. */
  @GetMapping("/{id}")
  public ResponseEntity<DiscountPlanDTO> getPlan(@PathVariable Integer id, Authentication auth) {
    requireRole(auth, User.Role.ADMIN, User.Role.MANAGER);
    return ResponseEntity.ok(discountPlanService.getPlanById(id));
  }

  /** POST /api/discount-plans/fixed Creates a fixed-rate discount plan (UC-8 support). */
  @PostMapping("/fixed")
  public ResponseEntity<DiscountPlanDTO> createFixedPlan(
      @Valid @RequestBody CreateFixedPlanRequest request, Authentication auth) {
  ...
  /** POST /api/discount-plans/flexible Creates a tiered (flexible) discount plan (UC-8 support). */
  @PostMapping("/flexible")
  public ResponseEntity<DiscountPlanDTO> createFlexiblePlan(
      @Valid @RequestBody CreateFlexiblePlanRequest request, Authentication auth) {
  ...
  /**
   * DELETE /api/discount-plans/{id} Deletes a discount plan by ID. Per the marking sheet: "delete
   * discount plan" (1 mark). The plan must not be currently assigned to any merchant.
   */
  @DeleteMapping("/{id}")
  public ResponseEntity<Void> deletePlan(@PathVariable Integer id, Authentication auth) {
    ...
    requireRole(auth, User.Role.ADMIN, User.Role.MANAGER);
    User actingUser = resolveUser(auth);
    discountPlanService.deletePlan(id, actingUser);
