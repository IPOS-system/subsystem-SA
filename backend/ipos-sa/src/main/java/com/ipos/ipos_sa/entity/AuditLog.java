 ...
 *
 * Every create / update / delete on sensitive entities (merchant accounts, orders, payments)
 * should produce an AuditLog entry. Records must never be updated or deleted.
 *
 * entity_name + entity_id together identify the affected business object, e.g.
 *   entity_name = "merchant", entity_id = 3  →  Merchant #3.
 */
@Entity
...
public class AuditLog {
    ...
    /**
     * High-level description of the action, e.g.
     * "CREATE_ACCOUNT", "UPDATE_CREDIT_LIMIT", "PLACE_ORDER", "RECORD_PAYMENT".
     */
