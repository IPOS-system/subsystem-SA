 ...
 *   PENDING  → OVERDUE  (due_date passed, auto-flagged by scheduled job)
 *
 * The merchant's account state (NORMAL / SUSPENDED / IN_DEFAULT) is driven by
 * how many days past due_date the invoice remains unpaid (see Merchant.AccountState).
 *
 * amount_due tracks the outstanding balance; it starts at (gross_amount - discount_amount)
 * and is reduced as Payment records are recorded by the accounting department.
 */
@Entity
@Table(name = "invoices")
@Getter
