 ...
 *
 * <p>Role rules enforced here: GET merchants / staff → ADMIN, MANAGER POST merchant account → ADMIN
 * only POST staff account → ADMIN only PUT merchant details → ADMIN, MANAGER PUT credit limit /
 * plan → ADMIN, MANAGER PUT deactivate / reactivate → ADMIN only PUT restore from IN_DEFAULT →
 * ADMIN, MANAGER, DIRECTOR PUT change role → ADMIN only
 */
@RestController
...
public class AccountController {
   ...
   *
   * <p>For MERCHANT role: returns the caller's own merchant profile (no params needed). For
   * ADMIN/MANAGER role: requires ?merchantId= query param to specify which merchant to look up.
   * This supports the CA subsystem calling getAccountStatus when logged in as an admin.
   */
  @GetMapping("/me")
  ...
  /** POST /api/accounts/merchant Creates a new merchant account. ADMIN only. */
  @PostMapping("/merchant")
  public ResponseEntity<MerchantDTO> createMerchant(
      @Valid @RequestBody CreateMerchantRequest request, Authentication auth) {
  ...
  /**
   * PUT /api/accounts/merchants/{id} Updates merchant contact details, credit limit, or discount
   * plan. ADMIN and MANAGER can update contact fields. Credit limit and discount plan changes also
   * require ADMIN or MANAGER.
   */
  @PutMapping("/merchants/{id}")
  public ResponseEntity<MerchantDTO> updateMerchant(
      @PathVariable Integer id, @RequestBody UpdateMerchantRequest request, Authentication auth) {
  ...
  /**
   * PUT /api/accounts/merchants/{id}/restore Restores a merchant from IN_DEFAULT to NORMAL. Per
   * marking sheet: ADMIN, MANAGER, and DIRECTOR can do this.
   */
  ...
  /** GET /api/accounts/staff Returns all non-merchant (staff) accounts. ADMIN only. */
  @GetMapping("/staff")
  ...
  /** POST /api/accounts/staff Creates a new staff account. ADMIN only. */
  @PostMapping("/staff")
  public ResponseEntity<StaffDTO> createStaff(
      @Valid @RequestBody CreateStaffRequest request, Authentication auth) {
  ...
  /** PUT /api/accounts/{id}/deactivate Soft-deletes any account (merchant or staff). ADMIN only. */
  @PutMapping("/{id}/deactivate")
  public ResponseEntity<Void> deactivate(@PathVariable Integer id, Authentication auth) {
  ...
  /** PUT /api/accounts/{id}/reactivate Re-activates a deactivated account. ADMIN only. */
  @PutMapping("/{id}/reactivate")
  public ResponseEntity<Void> reactivate(@PathVariable Integer id, Authentication auth) {
  ...
  /** PUT /api/accounts/{id}/reset-password Resets a user's password. ADMIN only. */
  ...
  /**
   * PUT /api/accounts/{id}/change-role Changes the role of an existing user account. ADMIN only.
   * Per the marking sheet: "change roles (promote/demote) to level of access" (2 marks).
   *
   * <p>This retains the user's username and password, which the marking sheet footnote explicitly
   * calls out as the advantage over delete+recreate.
   */
  @PutMapping("/{id}/change-role")
  public ResponseEntity<StaffDTO> changeRole(
      @PathVariable Integer id, @RequestParam User.Role newRole, Authentication auth) {
