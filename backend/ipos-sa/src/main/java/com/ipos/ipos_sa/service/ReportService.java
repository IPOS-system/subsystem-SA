...
public class ReportService {
   ...
   *
   * <p>Per the brief (Appendix 4 area / IPOS-SA-RPT item i): "Turnover for a given period of time
   * in terms of quantities of goods sold to merchants and the revenue received by InfoPharma Ltd."
   *
   * <p>Returns total revenue and a per-product breakdown of quantity sold and revenue, excluding
   * cancelled orders.
   */
   ...
   *
   * <p>Per the brief: "List of orders received from a particular merchant for a given period of
   * time with: order ID, date of ordering, value of the order, date of dispatching, payment
   * received (or pending), a 'Totals' line should conclude the report."
   */
   ...
   *
   * <p>Per the brief: "contact details of a merchant (as a header), list of all orders (including
   * the individual items ordered, quantity ordered, individual cost, total cost of order, discount
   * given if known, payment status)."
   */
  ...
  /**
   * Generates an invoice list for a specific merchant within a date range. Per the brief: "List of
   * invoices raised against an individual merchant for a given period."
   */
  ...
  /**
   * Generates an invoice list across all merchants within a date range. Per the brief: "List of all
   * invoices raised by InfoPharma Ltd against merchants for a given period."
   */
   ...
   *
   * <p>Per the brief: all active products below their minimum stock level, with a recommended
   * minimum order quantity calculated as: minStockLevel * (1 + reorderBufferPct / 100) -
   * availability
   *
   * <p>This brings stock up to the minimum level plus the buffer.
   */
   ...
   *
   * <p>Per the brief (IPOS-SA-RPT item vi): "Stock turnover (goods sold and newly received) within
   * a given period of time."
   *
   * <p>For each product, returns the total quantity sold to merchants during the period and the
   * current stock availability. Stock deliveries are tracked via the audit log (STOCK_DELIVERY
   * actions) but for the prototype we report on sales (from order_items) and current availability.
   */
   ...
   * @param includeMerchant if true, populates merchantId and merchantName (used by RPT-05
   *     all-invoices; null for RPT-04)
   */
   ...
   *
   * <p>Per Appendix 6 of the brief, there are two reminder types:
   * <ul>
   *   <li>FIRST reminder: sent when payment is 1–15 days overdue (account NORMAL or SUSPENDED)</li>
   *   <li>SECOND reminder: sent when payment is 15+ days overdue (account SUSPENDED or IN_DEFAULT)</li>
   * </ul>
   *
   * <p>The reminder text follows the Appendix 6 layout with merchant details, invoice reference,
   * and amount due.
   */
  ...
  /**
   * Builds the reminder letter text following the Appendix 6 layout.
   *
   * @param merchant  the debtor merchant
   * @param invoice   the overdue invoice
   * @param type      "FIRST" or "SECOND"
   * @param daysOverdue number of days past due date
   * @return formatted reminder text
   */
