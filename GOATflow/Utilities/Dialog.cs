...
namespace GOATflow
{
    ...
    // Edits: companyName, address, phone, fax, email, creditLimit, discountPlanId
    // All fields optional on PUT — only non-null values applied server-side
    ...
    // ── Add Product Dialog ──────────────────────────────────────────────────
    // Maps to CreateProductRequest:
    //   productId*, description*, packageType, unit, unitsPerPack*,
    //   unitPrice*, availability*, minStockLevel*, reorderBufferPct (default 10)
    public class AddProductDialog : Window
    {
        ...
        private async void Save_Click(object sender, RoutedEventArgs e)
        {
            ...
            { MessageBox.Show("Product code and description are required."); return; }
            if (!int.TryParse(_upp.Text, out int upp) || upp < 1)
            { MessageBox.Show("Enter valid units per pack."); return; }
            if (!decimal.TryParse(_price.Text, out decimal price) || price <= 0)
            { MessageBox.Show("Enter a valid unit price."); return; }
            if (!int.TryParse(_avail.Text, out int avail) || avail < 0)
            { MessageBox.Show("Enter valid availability."); return; }
            if (!int.TryParse(_minStock.Text, out int minStock) || minStock < 0)
            { MessageBox.Show("Enter a valid min stock level."); return; }
    ...
    // ── Edit Product Dialog ─────────────────────────────────────────────────
    // Maps to UpdateProductRequest — all fields optional
    ...
    // ── Dispatch Dialog ─────────────────────────────────────────────────────
    // Maps to DispatchOrderRequest: courier*, courierRef*, dispatchDate*, expectedDelivery*
    public class DispatchDialog : Window
    {
        ...
        private async void Dispatch_Click(object sender, RoutedEventArgs e)
        {
            ...
            { MessageBox.Show("Courier and reference are required."); return; }

            try
            {
                await Instance.HttpService.PutOrderDispatchAsync(
                    _orderId, _courier.Text, _ref.Text,
                    _dispatch.Text, _expected.Text);
